...
static const double R_EARTH = 6371004.0;


static void clear_rec(struct kdnode *node, void (*destr)(void*));
static int insert_rec(struct kdnode **node, const double *pos, int dir, int dim, int index);
...
double getDistBet2LatLong(const double *pos1, const double *pos2) {
	double factorX = 0.0, factorY = 0.0;
	factorX = cos(pos1[0] * M_PI / 180.0) * M_PI / 180.0 * R_EARTH;
	factorY = M_PI / 180.0 * R_EARTH;

	return SQ(factorX * (pos1[1] - pos2[1])) + SQ(factorY * (pos1[0] - pos2[0]));
}

double getDistBetXY(const double *pos1, const double *pos2) {
	return SQ((pos1[1] - pos2[1])) + SQ((pos1[0] - pos2[0]));
}
...
static int insert_rec(struct kdnode **nptr, const double *pos, int dir, int dim, int index)
{
	int new_dir;
	struct kdnode *node;
	if(!*nptr) {
	...
	if(pos[node->dir] < node->pos[node->dir]) {
		return insert_rec(&(*nptr)->left, pos, new_dir, dim, index);
	}
	return insert_rec(&(*nptr)->right, pos, new_dir, dim, index);
}

int kd_insert(struct kdtree *tree, const double *pos, const int index)
{
	if (insert_rec(&tree->root, pos, 0, tree->dim, index)) {
		return -1;
	}
	if (tree->rect == 0) {
...
static int find_nearest(struct kdnode *node, const double *pos, double range, struct res_node *list, int ordered, int dim)
{
	...
	// }
	dist_sq = getDistBet2LatLong(node->pos, pos);
	if(dist_sq <= SQ(range)) {
...
struct kdres *kd_nearest(struct kdtree *kd, const double *pos)
{
	...
	dist_sq = getDistBet2LatLong(result->pos, pos);
...
int kd_res_item(struct kdres *rset, double *pos)
{
	if(rset->riter) {
		if(pos) {
			...
			pos[2] = rset->riter->item->index;
		}
		return 0;//rset->riter->item->fileIndex;
	}
	return 0;
}
//
